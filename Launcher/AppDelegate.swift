...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @IBOutlet weak var hideFromMenuBarButton: NSButton!
    ...
    var hideFromMenuBar = NSControl.StateValue.off

    let icon = NSImage(named: "MenuIcon")
    let iconRunning = NSImage(named: "MenuIconRunning")

    override func awakeFromNib() {
        // Read preference first to determine if menu bar should be shown
        if let rawValue = prefs.string(forKey: "hideFromMenuBar") {
            hideFromMenuBar = NSControl.StateValue(rawValue: Int(rawValue) ?? 0)
        }
        
        // Always set up menu items (needed even if menu bar is hidden)
        ...
        // Only create menu bar item if not hidden
        if hideFromMenuBar != NSControl.StateValue.on {
    ...
    @IBAction func hideFromMenuBar(_ sender: NSButton) {
        hideFromMenuBar = hideFromMenuBarButton.state
        self.prefs.set(hideFromMenuBar == NSControl.StateValue.on ? "1" : "0", forKey: "hideFromMenuBar")
        
        // Update menu bar visibility immediately
        if hideFromMenuBar == NSControl.StateValue.on {
            // Hide menu bar item
            if menuBarItem.button != nil {
                statusBar.removeStatusItem(menuBarItem)
                menuBarItem = NSStatusItem()
            }
        } else {
            // Show menu bar item if it doesn't exist
            if menuBarItem.button == nil {
                menuBarItem = statusBar.statusItem(withLength: NSStatusItem.variableLength)
                menuBarItem.button?.title = ""
    ...
    func readPreferences() {
        ...
        hideFromMenuBarButton.state = hideFromMenuBar
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        // Only set menu bar icon if menu bar item exists
        if menuBarItem.button != nil {
